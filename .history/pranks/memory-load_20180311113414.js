...
class overloadMemory extends PrankBase {
  ...
  async overloadMemory(){
    ...
    console.log(fileContent.toString('utf8').substring(1, 100))
    global.state.push(fileContent);
    const currentMemorySizeInMB = sizeOf(global.state) / 1000 / 1000;
    console.debug(`Our fake object memory size is ${currentMemorySizeInMB} MB and the entire heap is ${util.process.memoryUsage()}`)
