...
class PrankBase extends EventEmitter {
  constructor(configuration, schedule, expressApp) {
    ...
    schedule.on("start", () => {
      console.log(`Prank ${this.context.configurationconfiguration.name} just received a start command from scheduler`);
      this.start();
    });
    schedule.on("stop", () => {
      console.log(`Prank ${this.configuration.name} just received a stop command from scheduler`);
      this.stop();
    });
  }
}
